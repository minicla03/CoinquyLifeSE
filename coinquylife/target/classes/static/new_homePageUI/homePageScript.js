...
let date = new Date();
const today = date.getDate(); // restituisce il giorno del mese corrente
const thisMonth = date.getMonth(); // restituisce il mese corrente (0-11)
...
function renderCalendar() {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        if(day === today && month === thisMonth) {
            calendarDays.innerHTML += `<div class="today">${day}</div>`;
        }
        else{
...
/*Logica per il popup del calendario*/

const popup = document.getElementById("calendar-dialog");
const wrapper = document.querySelector(".wrapper");


function addDaysListener(){
    const days = document.querySelectorAll(".day, .prev-days, .next-days");
    days.forEach(day => {
    ...
    for(let i=0; i<24; i++){
        ...
        selectOra.add(option);
    }
// Popola i minuti (0-59 a intervalli di 5 minuti)
    const selectMinuti = document.getElementById('minuti');
    for(let i=0; i<60; i+=1){
        let option = document.createElement('option');
        option.value = option.text = i.toString().padStart(2, '0');
        selectMinuti.add(option);
    }
}
