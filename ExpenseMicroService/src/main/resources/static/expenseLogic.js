...
function addExpense() {
    ...
    const selectedParticipants = document.getElementById("participants").value.split(",").map(p => p.trim()).filter(p => p);
    ...
    fetch('http://localhost:8084/rest/expense/createExpense', {
        method: 'POST',
        headers: {
...
async function retriveExpenses() {
    await fetch(`http://localhost:8084/rest/expense/getAllExpenses?houseId=house123`)
        .then(response => {
            if (!response.ok) {
...
{
    await fetch('http://localhost:8080/Auth/rest/auth/getUserByHouseId', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
