...
async function retriveExpenses() {
    await fetch(`http://localhost:8084/rest/expense/getAllExpenses?houseId=${encodeURIComponent(houseId)}`)
        .then(response => {
            if (!response.ok) {
...
document.getElementById("back-btn").addEventListener("click", async () => {
    try {
        const response = await fetch('https://localhost:8084/rest/client/backToHome', {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            }
